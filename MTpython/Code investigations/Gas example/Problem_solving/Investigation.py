...
# - Accent on sigma translated from yearly value to epochal values 
# 
# - Analogy to Brownian motion, used in the BSM model actuallly :D
# 
# from wiki [geometric brownian motion](https://en.wikipedia.org/wiki/Geometric_Brownian_motion)
...
def get_geom_wp_update(price_now, sigma, dt): 
    return price_now*np.exp((0-sigma**2/2)*dt+sigma*np.random.normal(0,np.sqrt(dt),1))


# In[548]:


get_geom_wp_update(20, 1, 1)
...
for i in range(epochs): 
    prices.append(price)
    price = get_geom_wp_update(price, sigma, dt)
