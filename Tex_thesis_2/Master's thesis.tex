...
In the whole thesis, bold capital letters, such as $\mathbf{X}$, represent a set of all elements $x \in \mathbf{X}$ as in \cite{Rol:18}. The cardinality of a set $\mathbf{X}$ is denoted with two vertical lines as $|\mathbf{X}|$. Random variables, understood in the sense of the standard Kolmogorov's probability theory \cite{Kol:60}, are represented with a tilde above the variable, e.g., $\tilde{x}$. Realizations of random variables are denoted by the same letters as the random variable without the tilde, e.g., $x$. 
...
We have chosen the class of projects that can be labeled as an investment in facilities with a simple I/O process and further managerial guidance. This class is characterized by a large outflow of money at the beginning, which is used for building the facility (or its first functional part) and a small but long-term positive future revenue driven by the difference in the price of inputs and outputs and the further managerial decisions. 
...
\subsection{Reward function}
The FCF model in our example, and actually in all projects in the class of facilities with a simple I/O process, is fairly straightforward.
...
\chapter{Conclusion}
The core message of this thesis is to interpret the problem of project valuation in the form of stochastic decision making. We can declare that the aforementioned contributions of the developed algorithm have been successfully presented by our experiment, most notably in contrast to the current valuation techniques as:
...
\begin{itemize}
	...
	\item preservation of the economic truths. 
\end{itemize}
