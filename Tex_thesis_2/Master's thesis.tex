...
Another very popular distribution in the ROA field is the log-normal distribution $LogN(\mu, \sigma)$. It represents a random variable, a logarithm of which is distributed normally. 
...
Another interesting example of the non-linear gain perception of individuals is the famous St. Petersburg paradox first formulated by Bernoulli in 1738 \cite{Ber:54}. This paradox is illustrated on a game with the following rules. A fair coin is being repeatedly tossed until the result is ``tails``, say in $n$-th toss. Then the reward for the participant is driven by the equation $2^{n-1}$, where for example the firs appearance of 	``tails`` in fourth toss rewards the participant with 8 units of currency.
