...
We begin by a clear definition of a project inspired by publications in the field of ROA. We define project valuation problem as a collection of mathematical constructs\textbf{ (?)} and identify the main limitations of this approach. 

Next, we focus on the identification of the project valuation in terms of SDT framework. We define all the relevant sets and functions to be able to talk about project valuation as a structured decision making under uncertainty. 
...
To be able to rigorously talk about the project valuation we need to define what is a \textit{project} and what do we mean by its \textit{valuation} in ROA. The inspiration for these definitions comes from examples and used rhetoric in the ROA publications, namely \cite{Gut:09}, \cite{Vol:03} and \cite{AmrKul:98}. 

None of the books that we have studied goes in detail to define a project as a collection of mathematical constructs, as for example SDT does with MDPs. Guthrie in \cite{Gut:09} illustrates what a project is with three initial examples and in each chapter presents a real investment opportunity in a way that it seems clear what is meant by this specific project, what are its parameters and rules and what is the goal of the investor. 

In other books like Vollert's \cite{Vol:03} and Kulatilaka's \cite{AmrKul:98} \textbf{(???)} a definition of a project is also not given, rather, it is assumed that the terms \textit{project} or \textit{capital investment} are clear. 

It is worth noting that a definition of project \textit{valuation} is also not deeply discussed in the ROA books. We feel like the term of valuation is assumed to be clear and a  is always being understood as expected net present value (NPV). 

NPV as a concept is easily understandable, however as we will discuss later it has some features that are not ideal, for example the unclear value of a discount rate or not reflecting the individual perception of risk. 

As explained above, nothing like a clear mathematical definition of a project valuation is presented in the ROA books. However, the used rhetoric is similar and we strongly believe that the project valuation can be understood as: "An amount of value that I am able to create with actions that can be considered as a part of one project, measured in a metric of expected net present value with a special non-axiomatic determination of the discount rate." \footnote{I am really not sure here, it does not seem that authors actually defined what a project valuation is... }
...
Solving a valuation problem in SDT means to define it as MDP, which consists of two parts. First there are three sets: time set $\mathbf{T}$, state set $\mathbf{S}$\footnote{This global state set is a union of state set in each time $\mathbf{S_t}$} and action set $\mathbf{A}$\footnote{This global action set is a union of action set in each state $\mathbf{A_{s_t} }$}, which express the structure of the decision making problem. The second part consists of two functions: transition probability function $p$ and reward function $r$, where $p$ is responsible for describing the stochastic evolution of the project and $r$ for informing about the value gains in each time epoch.

In the following paragraphs we will focus on each of these five important building blocks of the project valuation decision problem in detail. In each paragraph we will present an example from valuation of gas power-plant, helping the reader to understand and preparing the ground for our experiments that will follow. 

\subsection{Time set}
Even though the SDT theory is capable of handling infinite time horizons and continuous time modeling, these sophisticated formats are not needed for valuation of real life projects. The time dimension of a project can be reasonably described by a discrete set with known finite horizon for two reasons. 

First is that observing new information and making impactful decisions by project's manager is not done at any time but after some reasonable time chunks. No manager changes the course of a project 10 times a day\footnote{This thesis does not focus on the individual management of the internal processes of the project. This thesis focuses on managerial decisions that modify the project in a major way.}. 

Second reason is that managers do not think about projects as ever-lasting. Potential profits after certain time threshold are neglected. This is given either by the finite time-span nature of the projects (Gas power plant life-span) or the extreme uncertainty in modeling of cash flows (and their true values) in the far future. To model a project's cash flow in 100 years is a wishful thinking. 


Time intervals in our model reflect the frequency of influential management meetings at which the course of the project can be changed significantly, e.g. week, month or quarterly intervals. This notion is supported also by its usage in the ROA publications. 

\begin{example}
	Monthly decision time intervals in a duration of gas power plant lifespan which is 25 years. The time set is then $\mathbf{T} = \{0,1,2,...,300\}$.
\end{example}
...
The state set $\mathbf{S}$ can be constant, meaning that in each time epoch we are measuring the same parameters and the state vector has a constant length. However, it might be useful to think about dynamic state sets in each time epoch $t \in \mathbf{T}$ as $\mathbf{S_t}$, where for some reason the states have variable length\footnote{There are more or less relevant parameters to measure.}. 

It is worth noting that there are usually some elementary states that are influenceable by the managerial actions and some states that are not. In our notation we do not distinguish between them. 
...
\begin{example}
	Relevant features for a gas power plant might be for example: price of gas, price of CO2 allowances, price of power, installed capacity of the plant, debt to be repayed or if the plant is currently running or not. The first four elementary states would then be considered as uninfluenceable by our future actions, while the last three would not. 
\end{example}


\subsection{Action set}
In SDT structure the action set  $\mathbf{A}$ is usually understood as an actual set, however in the format of project valuation, we find it better to represent it as an action function, whose parameter is a  given state $s_t \in \mathbf{S_t}$ and output is a set of possible actions $\mathbf{A(s_t)}$. 

The reason for this is that possible managerial actions are most of the time strictly conditioned on the current status of the project itself. Only a small portion of all possible actions might be actually performed in a given state. 
...
The advantage of the SDT approach in contrast to ROA is that there is no theoretical  complication in adding an arbitrary amount of actions of any type (as classified in ROA by Guthrie \cite{Gut:09} for example) possibly even conditioned on one another. The only concern that needs to be reminded is that of computational complexity. 

\begin{example}
	Actions in gas power plant valuation might be to: build some stage of the plant if there is some stage left to built, run the plant if it has some installed capacity, mothball or sell the power plant for salvage value. We believe that this example clearly shows the dependency of possible actions on given state and why is it thus better to use the function notion from now on. 
\end{example} 

Now, we have defined the structure of a project, boundaries within which the project will evolve, managerial actions will be taken and value will be created. 


\subsection{Transition probability function}
Given the nature of projects the evolution from one state to another is a stochastic process. In this thesis we want to model the project as MDP and thus the probability distribution of the next state, described by so called transition probability function, is conditioned only on the previous state and the last action taken. In mathematical notation: 
\begin{equation}
	p(s_t|a_{t-1}, s_{t-1}).
\end{equation}
...
\end{equation}
where $s_t^i$ is the new elementary state of $s_t$\footnote{\textbf{(Maybe define better what is an elementary state somewhere above, is this even used? )}}. 
...
It is clear that in the majority of real-life projects, determining, or estimating, this transition probability function is a very hard, but crucial task. Decisions will be made based on its values increasing or decreasing the overall profitability of a project. 

The approach of ROA authors to modeling of these probabilities varies a lot. Some authors like Guthrie \cite{Gut:09}, \cite{ArmKul:98} \textbf{(???)} use the arbitrage principle to determine the probabilities of their binomial models. Some authors, like \cite{KulTri:94} use the principle of insufficient reason\footnote{Even though they do not call it that way. }, where they assign 50\% probability to movements in both directions of their binomial models. Some authors, like \cite{Gut:09} and \cite{Vol:03}, go also deeper in statistical modeling of the probabilities. 

Some details of how does SDT approach the estimation of $p(\cdot)$ will be discussed later in section \ref{}, however we must note that correct estimation of these probabilities is not the focus of this thesis.
...
As discussed in preliminaries, the notion of ``value`` is complicated. In our case of project valuation, the first approximation of the entity that is to be maximized is the free cash flow (FCF). FCF usually consists of expenses, which are a result of immediate managerial actions, and income, which tends to be a result of the environment (supply and demand for manufactured products or services) conditioned on a previous action or set of actions. 

Usually, the goal of MDP is to find a strategy, which maximizes the expected reward. However, as economical theory guides us, there is a clear preference in having money now instead of later (time value of money) and that investors do not value uncertain rewards the same as their expected values (risk-aversion of investors). Both of these economical phenomenons will influence the reward function that will be used to determine the best strategy. 

The details of the incorporation of these phenomenons will be discussed deeply in the following sections \ref{} and \ref{}. For now, lets declare that the reward function will be based on FCF, adjusted for individual preferences and abilities of the investor. 

The wide scope of the SDT theory encapsulates this phenomena of individual preferences in perception of value in term of utility. The ultimate goal is thus described as utility maximization. 

\begin{example}
	Reward function of a gas power plant is driven by its ability to make money by transforming the gas and CO2 allowances into the electrical power. The initial expenses for building the power plant result in extreme negative rewards (driven by action of building), while the profits are made as a multiple of installed capacity and the difference of input costs plus static costs and the revenue from selling the electricity (conditioned on action of running the plant in given period)
\end{example}

\bigskip

This paragraph concludes the basic identification of sets required by the SDT framework. In the next section, we will focus on the solution to the project valuation problem in detail. We will discuss the sources of transition probability function, the actual incorporation of time value of money and the risk-aversion of investors to the model. 

\section{Solution of the project valuation as SDT problem}
In the previous chapter, we focus on the basic structure of a project valuation understood as MDP. In this chapter we will go deeper and we focus on the details of the actual solution of such valuation problem. 

We begin this section by looking in detail at the estimation of the transition probability function. There we outline how the ROA ideas can be incorporated and how the SDT theory can help with more advanced estimation techniques. 

Then, we follow with incorporation of the economical truths - time value of money and risk-aversion of investors in a form of utility maximization principle. We focus on implementation details with accent on applicability by real-life managers. 
...
In this thesis we focus on real-life projects. Such projects are from their nature stochastic and except for some edge cases, the laws guiding their evolution are unknown and complex. Our search for the optimal strategy is based on the assumption that we have some model estimating the future paths of the project and we act as if this model was the reality. In our case this model is materialized in a form of transition probability function $p$, where for example one of the assumptions is the Markovian feature of the states. 

There are many ways how to model $p$. Let's first discuss the widely used techniques in ROA and how they can be translated into the SDT terminology. 


\paragraph{Risk neutral probabilities}
The idea of risk neutral probabilities comes from the capital asset pricing model and is the main modeling force in \cite{Gut:09} and \ref{}\textbf{(???)}. Based on the assumption of no-arbitrage principle, we can compute the probabilities of up and down movement in the binomial models as a function of the growth rate of the market, risk-free investment rate and so called risk-premium related to our specific investment domain. 

In the SDT terminology, this input could be understand as ``expert knowledge`` where the first expert is the one (usually the economist using the CAPM formula) who determines the inputs like the risk-premium, or market growth. In this philosophy of experts, the second expert is the market together with the no-arbitrage principle. 

This techniques is used in the binomial model, but could be also used for its limit case, which is the Poisson distribution.

\paragraph{Insufficient reasons}
The second widely used strategy used for modeling the probabilities, mainly in the binomial model again, is to assign the same 0.5 probability to both up and down move. 

This approach is in SDT called a principle of insufficient reasons, where the argument is that if there is no reason for expecting one outcome to be more probable that the other one, we should model them with the same probability. 

The ROA authors, for example \cite{AmrKul:98} do not use this terminology, only the model itself. 


\paragraph{More complex ROA tactics}
\textbf{<Deeper research needed>}

In all the presented cases there is a flavor of ``expert knowledge``, supported by the actually measured data on various levels. The data analysis in the studied publications is rather simple, with the exception of ... 

\paragraph{SDT innovation}
Apart from the terminology and consistent approach outlined above, the SDT offers another frameworks for modeling $p$. We digress and remind only the most influential and innovative approaches which are Bayesian learning and "information mixing". 

Bayesian learning, as described in preliminaries allows for an easy update of parameter estimation based on newly incoming data. For example starting experts opinion about the evolution of input prices, wee are able to update our model by Bayesian formula without asking the mentioned expert over and over again. 

Also, we would like to outline the possibilities of SDT in form of "information mixing", which is an innovative ...
...
We called this entity present cash equivalent. It represents the amount of money that an investor is logically indifferent to having instead of a future free cash flow. 

In preliminaries, we argue, that given a constant borrowing and risk-free investment rate of an investor, the present cash equivalent of a free cash flow vector is unique and always defined. 
...
\begin{equation}
V(s_{t-1}) = \max_{a_{t-1} \in \mathbf{A_{s_{t-1}}}}\sum_{s_t \in \mathbf{S_t}} p(s_t|a_t, s_{t-1}) PCE\left(r(s_t|a_t, s_{t-1})+V(s_t), s_{t-1}^d\right),
\end{equation}
where $s_{t-1}^d$ is the debt state in time $t-1$. 


\begin{remark}
	It needs to be clarified, that if we want to embrace the notion of ROA publications, where we assume the usual simply discounted free cash flow, we are able to do so with this framework as it is only a corner case, where the borrowing rate is the same as the risk-free rate. 
\end{remark}


\subsection{Risk-aversion of investors}
As discussed above, the nature of real-life projects is stochastic. The uncertain evolution of states results in the uncertain FCFs defined by thee function $r$, which has implications for the decision making of investors. 

As discussed in the preliminaries \ref{} the majority of investors is risk averse, meaning that they tend to value uncertain gains lower than is their expected value. 

We believe that this characteristic of investors is important to consider and that is why it we want to incorporate it by the well established SDT notion of utility. 
...
\begin{equation}
	V(s_{t-1}) = \max_{a_{t-1} \in \mathbf{A_{s_{t-1}}}}\sum_{s_t \in \mathbf{S_t}} p(s_t|a_t, s_{t-1}) \mu\left(PCE\left(r(s_t|a_t, s_{t-1})+V(s_t), s_{t-1}^d\right)\right),
\end{equation}


\begin{remark}
	Even though there are consistent methods for obtaining the utility function of the individual investors, it might be hard to get individual investor on board with the idea of utility\footnote{Investor might not have a time to answer the utility questionnaire as suggested by \cite{BacChi:19}.}. This does not present a fundamental problem for our valuation techniques, because we can always use the utility of the risk-neutral investor, which is unique and its usage supported by the lack of bias against uncertain outcomes. 
\end{remark}
...
As mentioned in the previous sections of this thesis, the classic DP algorithm can be used for computing the optimal strategy MDP's with rather small cardinalities of the $\mathbf{A}$, $\mathbf{S}$ and $\mathbf{T}$ sets. This known DP problem is literature addressed as ``three curses of dimensionality`` \cite{Pow:11}.
...
From the various possible ADP techniques, we have chosen the value iteration with parameter model approximation for two main reasons, which are coming from the fundamental features of projects. 

First is that the real-life projects tend to have large state spaces (even uncountable), while on the other hand the action set is usually limited. We cannot ask a manager to make a choice between 10 000 actions for example. This supports the choice of value iteration (?). 

Second is that we usually have a good intuition of what exactly in the given states ``makes money`` and we are able to identify and cluster the important parameters. This allows us to build a reasonable basis function set, which should allow for a good approximation of the value function.
...
Secondly, we have adjusted the MDP theory to respect the economical truths with the notion of present cash equivalent transformation respecting the time value of money and utility function interpretation of the risk-aversion of investors.

Lastly, we have advised the best ADP algorithm that copes with the problems of computational complexity of the usual DP solving algorithm for non-trivial project. 
...
In the previous chapter we have presented an algorithmic approach to a project valuation grounded on the SDT framework and its ideas. Now we want to illustrate the actual usage of this algorithmic approach on a chosen class of projects. 

We have chosen the class projects that can be labeled as investment in facilities with simple I/O process and further managerial guidance. This class is characterized by a large outflow of money in the beginning used for building the facility (or its first functional part) and a small but long-term positive future revenue driven by the difference in the price of inputs and outputs of the facility. 

This choice of class is supported by its appropriate level of complexity, allowing to show the power of the new algorithm while not being too complex, and its large representation in the project management world (?).

We could present this whole chapter using a general description of the chosen project class, however we believe that using only one specific representative will result in more clear picture of the situation. 

Our choice of investment into a gas power plant is based on three grounds. First, there is certainly an influence by Guthrie's example \cite{Gut:09}. Second, as we will see this valuation problem has reasonable dimensions, that allow for a good presentation of the valuation algorithm and third, the author of this thesis has a legitimate domain knowledge based on his short, but intensive work experience. 

In the first part of this chapter we will describe the valuation problem with the upmost precision. 

In the second part we will focus on the dependency of the project valuation and the amount of available managerial actions, allowed by its structure. 

In the final part of this experiment we will be comparing different setups of the project, with the aim to study the effects of focusing on different parts of the project setup in detail\footnote{So called sensitivity analysis}. First will be comparing projects with different time granularities (simulating more and less frequent managerial decision making) and finally we will study the effects of choosing different price models of inputs and outputs. 

In the first half of the experiment we await a confirmation of the ROA conclusion, that options (actions) add significant value. In the second half, we have no strong opinion about what the changes in project valuation will be. Overall it needs to be said that the main aim of this experiment is to present the algorithm, observe and describe its possible shortcomings and support its viability with a specific and rather complex modeling example. 



\section{General settings of the experiment}
First lets clarify in detail what we mean by the phrase investment in the gas power plant. We are positioning ourselves in a role of an investment analytic for a large utilities company\footnote{Companies that generate electric power usually provide also gas, water, sewage or other basic services.} company with a goal to evaluate the potential of building and managing a gas power plant. 

For simplicity, we are considering building only one or two 200MW blocks with expected life-span of 25 years \cite{}. We assume, that the price of each block is 65M EUR, which is an estimate based on \cite{}. We assume that the efficiency of the power plant will be ..., meaning that ... units of gas are needed to run one 200MW block at full capacity. Furthermore, we expect ... units of required CO2 allowances for the same capacity. 

In addition to the variable costs of CO2 and gas inputs we are counting with fixed costs of 3EUR/MW/hour(?) of installed capacity for maintenance and security. We are also allowing for a mothballed state, which cost we estimate for 500k EUR, with lowered maintenance and security costs to only 1 EUR/MW/hour (?).

Initially we assume that the power plant is being managed in monthly patters. Its power is being sold in monthly contracts in the beginning of each month and the needed gas and CO2 inputs are also being purchased at the same time. As was indicated earlier in this thesis, we do not want to go deep in the plant management and its internal processes. 

In this example we would also like to respect the influence of the government subsidies for renewable power plants and its effect on the volatility of the market prices. The details of which wee define below. 

Regarding the financing of the project, we assume that the majority of the initial payment will be made by a flexible loan\footnote{An ideally flexible loan where we can repay what we want when we want} with an interest rate of 6\% and the risk-free interest rate is 2\%. The reality of having non-zero initial funds is reflected in the corresponding elementary state. 

Now that we have clarified the project that we are to value, we can proceed with precise definitions of the MDP parts. 


\paragraph{Time set}
As outlined in the introduction of this chapter, we assume that the life-span of a gas power plant is 25 years. Also wee assume that the management works in monthly pattern. 
...
As such, we identify eight elementary states as parameter describing
\begin{itemize}
	\item price of electricity
	\item price of CO2 allowances
	\item price of gas 
	\item government support for renewables
	\item number of blocks built
	\item encoded state of the power plant in the last month 
	\item lifespan left (in months) of the first block 
	\item lifespan left (in months) of the second block 
	\item potential debt to be repayed
\end{itemize}
in the start of each time epoch, for example of each month. 

The states of the state set are defined with having a constant length, since there is no significant change in relevant parameters through the project. 

Mathematically the state set $\mathbf{S}$ is defined as: 
\begin{equation}
	\mathbf{S} = \{[s^1,...,s^9]|s^i \in \mathbf{S^i} for i \in(1,...,9))\}, 
\end{equation}
where $\mathbf{S^i}$ represents the limitation of the individual states. These limitations will be in detail discussed now. 

The limits for the prices, i.e.  $s^i$ with $i \in \{1,2,3\}$ we define: 
\begin{equation}
	\mathbf{S^i} = \mathbb{R_0^+}
\end{equation}


<State set, defined in "rectangular way", some states are logically unreachable. Vector with encoded state of the environment. For example [built/not-built plant, index of gas price increase/decrease, index of power price increase/decrease> 
