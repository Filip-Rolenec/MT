...
<Intro for derivatives and more light introduction to the topic> 
...
Through my studies at the FNSPE CTU, I have specialized in the theory of dynamic decision making under uncertainty. In the final years of the university I also became curious about the world of corporate finance, since I was working for a venture capital company. This combination of my interests is nicely reflected in the assignment of this thesis - interpretation of real options in the terms of SDT. 
...
\begin{itemize}
	\item financial analysis \cite{BerDeM:09};
	\item real options \cite{Gut:09};
	\item statistical decision theory \cite{BacChi:19}.
\end{itemize}
...
In the whole thesis, bold capital letters, such as $\mathbf{X}$, represent a set of all elements $x \in \mathbf{X}$ as in \cite{Rol:18}. The cardinality of a set $\mathbf{X}$ is denoted with two vertical lines as $|\mathbf{X}|$. Random variables, understood in the sense of the standard Kolmogorov's probability theory \cite{Kol:60}, are denoted with a tilde above the variable, e.g., $\tilde{x}$. Values of random variables are denoted by the same letters as the random variable without the tilde, e.g., $x$. 
...
\end{definition}
\begin{remark}
	To rigorously unify the notation and simplify the formulas, a Radon-Nikodym (RN) density \cite{Rao:87a} is introduced  with the notation $p(x)$ and the name ``probability density``. The dominating measure of this RN density is either the counting measure (in discrete case) or the Lebesgue measure (in continuous case). 
\end{remark}
...
\begin{definition}(Conditional density) 
	Let symbol $p(x|y)$ represent the conditional density of a random variable. Then the $p(x|y)$ is defined as:
...
\begin{remark}
	The definition of conditional density expressed by the equation (\ref{eq:condP}) corresponds with the classic definitions of the conditional probability mass function and conditional probability density function in both the discrete and continuous case. 
\end{remark}
...
The simplest non-trivial distribution is the Bernoulli distribution $Be(p)$, which models the probability of success (1) or failure (0) with $p \in [0,1]$ by the p.m.f.: 
...
Its probability p.m.f. given parameter $\lambda$ is: 
\begin{equation}
...
The binomial model is one of the simplest models of variable evolution \cite{Gut:09}. The idea is that there is some constant probability of an up movement $p_u$ (in price, quantity, etc.) and a down movement $p_d$ of the variable. Each move then updates the variable's value by multiplying it by a given coefficient $U$ or $D$. To ensure that the number of possible states rises linearly and not exponentially with the number of time epochs, the model limits the parameters as $D=1/U$.

Based on the number of up movements $n_u$, the time epoch number $t$, and the initial value $v_{init}$, the expression of variables value is fairly simple: 

\begin{equation}
	v = v_{init} \cdot U^{2n_u-t}.
\end{equation}

The distribution of $n_u$ in time $t$ is then described by a binomial distribution $Bi(t,p_u)$.
...
\section{Valuation in Finance}

This thesis is built on two main theoretical pillars. The first is the principles of corporate finance \cite{BerDeM:09}, which culminates in real option analysis (ROA), and the second is statistic decision theory (SDT) \cite{BacChi:19}. A basic review of corporate finance terminology is presented in this section, focusing on project valuation techniques. 
...
\begin{remark}
	The M in BSM model stands for Merton, who expanded on the article \cite{BlaSch:73} with his article \cite{Mer:73} and received the Nobel price together with Scholes. \footnote{I want to talk about it because I want to explain why BSM and not BS model, where I do not want to use BS model for obvious reasons. }
\end{remark}
...
The SDT's primary focus is to determine the optimal strategy (a sequence of decisions) to act upon, generally in a dynamic and uncertain environment. In this thesis, we will be modeling the decision-making problems using the standard framework of the Markov Decision Process (MDP) \cite{Put:05}. 
...
\subsection{Dynamic programming/Reinforcement learning}\label{Sec:DP}
...
\begin{remark}
	For a constant number of states and actions in each time epoch, this reduction is from $|\mathbf{A_{s_t}}|^{|\mathbf{S_t}|^{|\mathbf{T}|}}$  to $|\mathbf{A_{s_t}}|\cdot |\mathbf{S_t}|\cdot|\mathbf{T}|$.
\end{remark}
...
\begin{itemize}
	...
	\item in each $s_t \in \mathcal{S}_t$ the optimal action $a_t^*$ is determined as argument maximizing the Bellman equation \ref{eq:Bellman}, where the future value function is understood as its approximation from the previous step\footnote{Or in case of the last epoch $t$ it is defined.}; 
